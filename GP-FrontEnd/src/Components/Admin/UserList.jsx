...
export default function UserList() {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      const token = sessionStorage.getItem('token');
      const response = await axios.delete(`http://127.0.0.1:4000/deleteUser/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log("s2",userId);
      ...
      <div className="mx-auto max-w-screen-lg px-4 py-8 sm:px-8">
        <div className="flex items-center text-center justify-between pb-6 usersmgn">
          ...
          <div className="flex items-center justify-between">
            <div className="ml-10 space-x-8 lg:ml-40 mr-6">
              <input
                className="block w-full py-3 pl-8 pr-4 text-base font-medium leading-normal bg-white border border-solid outline-none appearance-none placeholder:text-secondary-dark peer text-stone-500 border-stone-200 bg-clip-padding rounded-2xl"
                placeholder="Search..."
                type="text"
                value={searchTerm}
                      ...
                      <td className="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                        <div className="flex items-center">
                          <div className="h-10 w-10 flex-shrink-0">
                            {console.log("dhbgi",user)}
{user.profilePhotoUrl ? (
                        <img className="w-[45px] h-[40px] shrink-0 inline-block rounded-xl" src={user.profilePhotoUrl} alt={img1} />
                    ) : (
                       <span>  <img className="w-[50px] h-[40px] shrink-0 inline-block rounded-xl" src={img1} alt={img1} />
 </span>
                    )}                          </div>
                          <div className="ml-3">
