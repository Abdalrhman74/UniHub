    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (isEditing) {
          ...
          const response = await axios.post(`http://127.0.0.1:4000/post/create/${courseId}`, values, {
            headers: { Authorization: `Bearer ${token}` }
          });
          toast.success('Post created successfully');
          console.log(values.tag);
          console.log("sss",response.data.result.message , response.data.result.postId);
          if(response.data.result.message === "Post created successfully" && (values.tag === "Question" || values.tag === "question" )  )
          {

            const postId = response.data.result.postId;
            const response2 = await axios.post(`http://127.0.0.1:4000/posts/${postId}/comment`, {
              headers: { Authorization: `Bearer ${token}` }
            });
            if(response2.data.message ==="Comment added successfully"){
              fetchPosts();
              fetchComments();
            }
  ...
  return (
    ...
    <div className="mt-4">
              ...
              {posts.length > 0 ? (
                posts.map((post) => (
                  <div key={post.postId} ref={el => (postRefs.current[post.postId] = el)} className="bg-white rounded shadow mt-5 ddjj relative">
                    <h1 className="font-bold text-gray-600 text-center pt-4">{post && post.userDetails && post.userDetails.firstName} {post && post.userDetails && post.userDetails.lastName}</h1>  
      ...
      componentDecorator={(decoratedHref, decoratedText, key) => (
        <a href={decoratedHref} key={key} className="linkify-link" target='_blank'>
          {decoratedText}
        </a>
      )}
    >   
                                ...
                                {comments[post.postId].map((comment) => (
                                  <div key={comment.commentId} className="mb-4">
                                    <div className="flex items-start">
                                      ...
                                      <div>
                                      <div className="font-bold text-gray-600">{comment.userDetails && comment.userDetails.firstName} {comment.userDetails && comment.userDetails.lastName}</div>                                        <div className="text-sm text-gray-500">
                                          <i className="fa fa-clock-o"></i> {comment.userDetails && comment.commentDate.slice(0, 10)}
                                          <br />
                                          <i className="fa fa-clock-o"></i> {comment.userDetails && comment.commentDate.slice(11, 16)}
                                        </div>
    ...
    >                                          {comment.userDetails && comment.commentBody}
                                          </Linkify>
                                        </div>
                                        <button
