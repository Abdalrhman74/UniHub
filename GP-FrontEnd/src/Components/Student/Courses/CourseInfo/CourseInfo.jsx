  ...
  useEffect(() => {
    ...
    const fetchCoursePhoto = async () => {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:4000/course-photo/${courseId}`,{
          headers: { Authorization: `Bearer ${token}` }
      });
      setCoursePhoto(response.data.coursePhoto);
      } catch (error) {
    ...
    try {
      ...
      const response = await axios.post(
        'http://127.0.0.1:4000/courses/register',
        {
          ...
          courseCode: course?.course?.courseCode, // Ensure course data is available
        },
        {
          headers: {
        ...
        <div className="colorofinfo p-4 rounded-xl shadow-md sourceof">
          ...
          <div className='hellocontainer mt-8'>
            ...
            <div className='helloimg'>
              <img src={coursePhoto} alt="Course Photo" />
            </div>
          </div>
