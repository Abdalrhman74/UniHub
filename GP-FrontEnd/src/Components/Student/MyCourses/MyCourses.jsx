...
const MyCourses = () => {
    ...
    useEffect(() => {
        ...
        const fetchCourses = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:4000/courses/registered', {
                    headers: {
                        Authorization: `Bearer ${sessionStorage.getItem('token')}`, // Assuming token stored in sessionStorage
                    },
                });
                console.log('API Response (Registered):', response.data);
        
                if (response.data && Array.isArray(response.data.courses)) {
                    setCourses(response.data.courses);
                } else {
                    console.error('Unexpected response format (Registered):', response.data);
                    setCourses([]);
                }
            } catch (error) {
                console.error('Error fetching courses (Registered):', error);
                setCourses([]);
            }
        };
        ...
        const fetchRegistrationStatuses = async () => {
            try {
                ...
                for (const course of courses) {
                    const response = await axios.get(`http://127.0.0.1:4000/courses/${course.courseID}/status`, {

                        headers: { Authorization: `Bearer ${token}` }
                    });

                    
                    statuses[course.courseID] = response.data.courseStatus === 0 ? "Active" : "Archived";
    ...
    }, [courses.length]); // Adding courses.length to the dependency array
    ...
    const handleArchiveCourse = async (courseId) => {
        try {
            const token = sessionStorage.getItem('token');
            await axios.put(`http://127.0.0.1:4000/courses/${courseId}/archive`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
                    ...
                    <div class="relative flex flex-col break-words min-w-0 bg-clip-border rounded-xl bg-neutral-900 mb-5">
                        ...
                        <div class="flex-auto block py-8 px-9">
                            <div class="m-0 z-20 relative">
                                ...
                                <Link to={'/Home'} class="inline-block shrink-0 text-base font-medium leading-normal text-center align-middle cursor-pointer rounded-[.95rem] transition-colors duration-150 ease-in-out text-white bg-white/[.15] shadow-none [border:_0]  px-5 py-3.5 hover:bg-white/25 active:bg-white/25 focus:bg-white/25

                ">All Courses</Link>

                
                                    <Link to={'/MyCourses'} class="shrink-0 inline-block text-base font-medium leading-normal text-center align-middle cursor-pointer rounded-[.95rem] transition-colors duration-150 ease-in-out text-dark bg-white border-white shadow-none [border:_0]  px-5 py-3.5 hover:bg-white/90 active:bg-white focus:bg-white
                ">My Courses</Link>
                                </div>
                            </div>
                    ...
                    <tbody>
                        {currentCourses.map((course) => (
                            <tr key={course.courseID} className="border-b border-dashed last:border-b-0">
                                ...
                                <span 
        className={`text-center align-baseline inline-flex px-4 py-3 mr-auto items-center font-semibold text-[.95rem] leading-none rounded-lg ${registrationStatuses[course.courseID] === 'Active' ? 'text-success bg-success-light hover:bg-warning-light hover:text-warning cursor-pointer' : 'text-warning bg-warning-light'}`}
        onClick={() => registrationStatuses[course.courseID] === 'Active' && handleArchiveCourse(course.courseID)}
        onMouseEnter={(e) => { 
            if (registrationStatuses[course.courseID] === 'Active') {
