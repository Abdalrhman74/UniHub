...
import { faFilePdf, faFileWord, faFilePowerpoint, faFileVideo, faFileImage, faFile, faStar } from '@fortawesome/free-solid-svg-icons';
    ...
    useEffect(() => {
        ...
        const fetchCourseRating = async () => {
            try {
                ...
                setAverageRating(isNaN(rating) ? 0 : rating); // Convert to number and handle NaN
...
const handleDelete = async (materialId) => {
    try {
        ...
        const response = await axios.delete(`http://localhost:4000/material/delete/${materialId}`, {
            headers: { Authorization: `Bearer ${token}` }
        });

        if (response.status === 200 && response.data.message === 'Material deleted successfully') {
                    ...
                    <div class="border-black/12.5 shadow-soft-xl relative flex h-full min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border p-4 mb-4">
                        <div class="relative h-full overflow-hidden bg-cover py-6 rounded-xl" style={{backgroundImage: "url('https://images.unsplash.com/photo-1655635643532-fa9ba2648cbe?ixlib=rb-1.2.1&amp;ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&amp;auto=format&amp;fit=crop&amp;w=2232&amp;q=80')"}}>
                            ...
                            <div class="relative z-10 flex flex-col flex-auto h-full p-4">
                                <h1 class="pt-2 mb-6 font-bold text-white text-3xl ">{course && course.course.courseName} </h1>
                                <h2 class="mt-auto mb-0 font-semibold leading-normal text-white group text-2xl" >
                                    Lets's Discover More
                                    <i class="fas fa-arrow-down ease-bounce text-size-sm group-hover:translate-x-1.25 ml-1 leading-normal transition-all duration-200" aria-hidden="true" data-selected="selected-icon-hover"></i>
                                </h2>
                                <h2 className='flex items-center text-white text-xl mt-8 '>Rating:
                                        <div className="flex items-center">
                                            <FontAwesomeIcon icon={faStar} className="text-yellow-400 mr-1" style={{ color: 'yellow' }} />
                                        <span className="text-yellow-400">{averageRating && averageRating.toFixed(2)}</span>
                                            </div>
                                    </h2>
                            </div>
                        </div>
                            ...
                            <div className="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl px-3 py-3 rounded-2xl bg-clip-border">
                                ...
                                <div className="flex-auto px-1 pt-6">
                                    ...
                                    <a href={material.link}>
                                        <h5 className='text-xl'>{material.materialDesc}                                                                                                                         
                                        </h5>
                                        <span className='justify-content-end'>{material.uploadDate.slice(0,10) + " " + material.uploadDate.slice(11,16)}</span>
                                    ...
                                    <a href={material.link} className="" target='_blank'>
                                        ...
                                        >
                                            <i className="fas fa-trash trash-icon ml-0"></i>
                                        </button>
