...
const InstructormaterialSidebar = () => {
    ...
    useEffect(() => {
        ...
        if (path === `/InstructorMaterials/${courseId}`) {
            setActiveLink(`/InstructorMaterials/${courseId}`);
        } else if (path === '/InstructorSettings') {
            setActiveLink('InstructorSettings');
        } else if (path === `/UploadMaterial/${courseId}`) {
            setActiveLink(`/UploadMaterial/${courseId}`);
        } else if (path === `/InstructorPosts/${courseId}`) {
            setActiveLink(`/InstructorPosts/${courseId}`);
        }
    }, [location, courseId]); // Run effect when location or courseId changes
            ...
            <aside className="group/sidebar flex flex-col shrink-0 lg:w-[300px] w-[250px] transition-all duration-300 ease-in-out -translate-x-full lg:translate-x-0 m-0 fixed z-40 inset-y-0 left-0 lg:bg-light/30 bg-light border-r border-r-dashed border-r-neutral-200 sidenav fixed-start loopple-fixed-start" id="sidenav-main">
                <div className="h-full bg-neutral-900">
                    ...
                    <div className="flex items-center justify-between px-8 py-5">
                       ...
                       <span>  <img className="w-[45px] h-[40px] shrink-0 inline-block rounded-xl" src={img1} alt={img1} />
 </span>
                    )}                                 </div>
                            </div>
                            <div className="mr-2">
                                <a href="javascript:void(0)" className="dark:hover:text-primary hover:text-primary transition-colors duration-200 ease-in-out text-[1.075rem] font-medium dark:text-neutral-400/90 text-white/90 hover:text-white">
                                    {userData && `${userData[0].firstName} ${userData[0].lastName}`}
                                </a>
                                <span className="text-secondary-dark dark:text-muted font-medium block text-[0.95rem]">
                                    {userData && userData[0].userType}
                                </span>
                            </div>
                        </div>
                        <Link to={'/InstructorSettings'} className="inline-flex relative items-center group justify-end text-base font-medium leading-normal text-center align-middle cursor-pointer rounded-xl transition-colors duration-150 ease-in-out text-dark bg-transparent shadow-none border-0">
                        ...
                        <div className="flex flex-col w-full font-medium">
                            ...
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={`/InstructorMaterials/${courseId}`} className={`flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 hover:text-white ${activeLink === `/InstructorMaterials/${courseId}` ? 'text-white' : 'text-muted'}`} onClick={() => handleSetActiveLink(`/InstructorMaterials/${courseId}`)}>Materials</Link>
                                </span>
                            </div>
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={`/UploadMaterial/${courseId}`} className={`flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 hover:text-white ${activeLink === `/UploadMaterial/${courseId}` ? 'text-white' : 'text-muted'}`} onClick={() => handleSetActiveLink(`/UploadMaterial/${courseId}`)}>Upload</Link>
                                </span>
                            </div>
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={`/InstructorPosts/${courseId}`} className={`flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 hover:text-white ${activeLink === `/InstructorPosts/${courseId}` ? 'text-white' : 'text-muted'}`} onClick={() => handleSetActiveLink(`/InstructorPosts/${courseId}`)}>Posts</Link>
                                </span>
                            </div>
                            ...
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl ">
                                    <Link to={'/InstructorHome'} className="flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 text-muted hover:text-white"> Return Home</Link>
                                </span>
                            </div>
                            {/* <!-- menu item --> */}
                        </div>
...
export default InstructormaterialSidebar;