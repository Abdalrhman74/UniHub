...
const InstructorSettings = () => {
    ...
    const handlePhotoChange = async (event) => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const response = await axios.post('http://127.0.0.1:4000/upload-photo', formData, {
                headers: {
        ...
        <div className="settings-page">
                        ...
                        <div className="flex flex-col w-full font-medium">
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={'/InstructorSettings'} className="flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 text-muted hover:text-white text-white">
                                        Profile
                                    </Link>
                                </span>
                            </div>
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={'/InstructorPrivacy'} className="flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 text-muted hover:text-white">Privacy</Link>
                                </span>
                            </div>
            ...
            <div className="bg-gray-100 dark:bg-gray-800 transition-colors duration-300 personalInformationContainer">
                <div className="container p-4">
                    <div className="changeprofilepicturephoto relative " onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)}>
                        <img src={profilePhotoUrl } className="w-[80px] h-[80px] rounded-full object-cover" alt={userData.firstName} loading="lazy" />
                        {hovering && (
                            <div className=" w-[80px] h-[80px] absolute inset-0 bg-gray bg-opacity-50 flex items-center justify-center text-white text-sm font-semibold rounded-full cursor-pointer" onClick={() => document.getElementById('fileInput').click()}>
                                Change Photo
                            </div>
                        )}
                    </div>
                    <input type="file" id="fileInput" style={{ display: 'none' }} onChange={handleFileSelect} />
                    <button onClick={handlePhotoChange} className="mt-2 bg-blue-500 text-black py-1 px-2 rounded changeprofilepicturephoto pl-4">Upload</button>
