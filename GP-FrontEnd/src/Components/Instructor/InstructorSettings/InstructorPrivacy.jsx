...
const InstructorPrivacy = () => {
    ...
    const fetchProfilePhoto = async () => {
        try {
            ...
            if (response.data && response.data.profilePhotoUrl !== 'null') {
    ...
    const editUserProfile = async (values) => {
        try {
            const token = sessionStorage.getItem('token');
            let formattedBDate = userData.bDate.split('T')[0]; // Removing time component
            const response = await axios.put('http://127.0.0.1:4000/editUser', { ...userData, bDate: formattedBDate , password: values.password }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    return (
        <div className="settings-page">
            <aside className="group/sidebar flex flex-col shrink-0 lg:w-[300px] w-[250px] transition-all duration-300 ease-in-out -translate-x-full lg:translate-x-0 m-0 fixed z-40 inset-y-0 left-0 lg:bg-light/30 bg-light border-r border-r-dashed border-r-neutral-200 sidenav fixed-start loopple-fixed-start" id="sidenav-main">
                <div className="h-full bg-neutral-900">
                    ...
                    <div className="flex items-center justify-between px-8 py-5">
                       ...
                       <span>  <img className="w-[45px] h-[40px] shrink-0 inline-block rounded-xl" src={img1} alt={img1} /></span>
                    )}                                </div>
                            </div>
                        ...
                        <div className="flex flex-col w-full font-medium">
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={'/InstructorSettings'} className="flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 text-muted hover:text-white">Profile</Link>
                                </span>
                            </div>
                            <div>
                                <span className="select-none flex items-center px-4 py-3 cursor-pointer my-[.4rem] rounded-xl">
                                    <Link to={'/InstructorPrivacy'} className="flex items-center flex-grow text-[1.15rem] dark:text-neutral-400/75 text-muted hover:text-white text-white">Privacy</Link>
                                </span>
                            </div>
            ...
            <div className="bg-gray-100 dark:bg-gray-800 transition-colors duration-300 main-change-password-container">
                <div className=" container p-4" >
                    <div className=" bg-gray dark:bg-gray-700 shadow rounded-lg p-6 change-password-container ">
                        <h1 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100 pb-8">Change Password</h1>
                            ...
                            >
                                {({ isSubmitting }) => (
                                    <Form>
                                        ...
                                        <button type="submit" disabled={isSubmitting} className="bg-blue-500 text-black p-3 rounded changePasswordbtn">
                                            Change Password
                                        </button>
                                    </Form>
                                )}
...
export default InstructorPrivacy;
