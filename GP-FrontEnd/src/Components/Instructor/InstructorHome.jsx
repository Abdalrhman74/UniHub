...
const InstructorHome = () => {
    ...
    const [instructorStatistics, setInstructorStatistics] = useState(null); // New state for course statistics
    const [instructorStatistics2, setInstructorStatistics2] = useState(null); // New state for course statistics
    ...
    const fetchCourseRating = async (courseId) => {
        try {
            ...
            const response = await axios.get(`http://localhost:4000/course/${courseId}/viewrate`, {
                headers: { Authorization: `Bearer ${token}` }
            });


            const rating = parseFloat(response.data.averageRating);
    ...
    useEffect(() => {
        ...
        const fetchCourses = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:4000/course/instructor', {
                    headers: {
                ...
                if (response.data && Array.isArray(response.data)) {
                    const coursesWithRatings = await Promise.all(
                        response.data.map(async (course) => {
                            const rating = await fetchCourseRating(course.courseID);
                            return { ...course, averageRating: rating.toFixed(2) };
                        })
                    );
        ...
        const fetchTasks = async () => {
            try {
                const token = sessionStorage.getItem('token'); // Adjust if you store the token differently
        ...
        const fetchInstructorStatistics = async () => {
            try {
                const token = sessionStorage.getItem('token');
                const response = await axios.get('http://127.0.0.1:4000/instructor-courses-count', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                if (response.data.status === 200 ) {
                    setInstructorStatistics(response.data);
                } else {
        ...
        const fetchInstructorStatistics2 = async () => {
            try {
                const token = sessionStorage.getItem('token');
                const response = await axios.get('http://127.0.0.1:4000/instructor-user-count', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                console.log("bxvisihigiu",response.data);
                if (response.data.status === 200 ) {
                    setInstructorStatistics2(response.data);
                } else {
                    setError('Failed to fetch Instructor statistics');
                }
            } catch (error) {
    ...
    useEffect(() => {
        if (query) {
            const fetchSuggestions = async () => {
                try {
                    const token = sessionStorage.getItem('token');
                    const response = await axios.get('http://127.0.0.1:4000/search-courses', {
                        headers: { Authorization: `Bearer ${token}` },
                        params: { course: query }
                    });

                    console.log("ota1",courses);

                    console.log("ota2",response.data);
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                ...
                if (response.data) {
                    // Filter notifications for admin users
                    const filteredNotifications = response.data.filter(notification => notification.userType === 'admin');
                    // Sort filtered notifications by postDate in descending order
                    const sortedNotifications = filteredNotifications.sort((a, b) => new Date(b.postDate) - new Date(a.postDate));
    ...
    {suggestions.length > 0 && (
      <ul class="absolute w-full bg-white border border-stone-200 rounded-lg shadow-lg mt-12 z-10">
        {suggestions.map((course) => (
          <li key={course.courseID} class="py-2 px-4 hover:bg-gray-100 cursor-pointer">
           <Link to={`/InstructorMaterials/${course.courseID}`} > {/* ------>  want to modify */}
          {course.courseName} ({course.courseCode}) 
                    ...
                    <div class="relative flex items-center ml-2 lg:ml-4">
                        <Link to={'/InstructorSettings'} class="flex items-center justify-center w-12 h-12 text-base font-medium leading-normal text-center align-middle transition-colors duration-150 ease-in-out bg-transparent border border-solid shadow-none cursor-pointer rounded-2xl text-stone-500 border-stone-200 hover:text-primary active:text-primary focus:text-primary ">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        ...
                        <div class="flex-auto block py-8 px-9">
                            <div class="m-0 z-20 relative">
                                ...
                                <div class="flex flex-col gap-2 xl:gap-4 sm:flex-row">
                                    ...
                                    <Link to={'/InstructorHome'} class="shrink-0 inline-block text-base font-medium leading-normal text-center align-middle cursor-pointer rounded-[.95rem] transition-colors duration-150 ease-in-out text-dark bg-white border-white shadow-none [border:_0]  px-5 py-3.5 hover:bg-white/90 active:bg-white focus:bg-white
                ">My Courses</Link>
                                </div>
                            </div>
                    ...
                    <div class="relative flex flex-col min-w-0 break-words border border-dashed bg-clip-border rounded-2xl border-stone-200 bg-light/30 mb-5">
                        ...
                        <div class="px-9 pt-5 flex justify-between items-stretch flex-wrap min-h-[70px] pb-0 bg-transparent">
                            <h3 class="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark">
                                <span class="mr-3 font-semibold text-dark">My Courses</span>
                                <span class="mt-1 font-medium text-secondary-dark text-lg/normal">Check some of your courses</span>
                            </h3>
                ...
                <table className="w-full my-0 align-middle text-dark border-neutral-200">
                    <thead className="align-bottom">
                        <tr className="font-semibold text-[0.95rem] text-secondary-dark">
                            ...
                            <th className="pb-3 pr-12 text-end min-w-[175px]">Rating</th>
                        ...
                        {currentCourses.map((course) => (
                            <tr key={course.courseID} className="border-b border-dashed last:border-b-0">
                                ...
                                <td className="p-3 pr-0 text-end">
                                    <span className="font-semibold text-light-inverse text-md/normal">{userData && userData[0].firstName + " " + userData[0].lastName }</span>
                                </td>
                                <td className="p-3 pr-0 text-end">
                                ...
                                <td className="p-3 pr-12 text-end">
                                    <span className={`text-center align-baseline inline-flex px-4 py-3 mr-auto items-center font-semibold text-[.95rem] leading-none rounded-lg text-primary bg-primary-light`}>
                                ...
                                <td className="p-3 pr-0 text-end">
                                <Link to={`/InstructorMaterials/${course.courseID}`}>
                    ...
                    <div class="relative flex flex-col min-w-0 break-words bg-gray-100 border-0 bg-clip-border rounded-2xl mb-5">
                        ...
                        <div class="px-9 pt-5 flex justify-between items-stretch flex-wrap min-h-[70px] pb-0 bg-transparent">
                            <div class="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/normal text-dark">
                                {/* <!--begin::Amount--> */}
                                <span class="text-dark text-5xl/none font-semibold mr-2 tracking-[-0.115rem]">{instructorStatistics && instructorStatistics.courseCount}</span>
                                {/* <!--end::Amount--> */}

                                {/* <!--begin::Subtitle--> */}
                                <span class="pt-1 font-medium text-dark text-lg/normal">Total Courses</span>
                                {/* <!--end::Subtitle--> */}
                            </div>
                        </div>
                        {/* <!-- card body  --> */}
                        <div class="flex items-end flex-auto py-8 pt-0 px-9 ">
                            ...
                            <div class="flex flex-col items-center w-full mt-3">
                                ...
                                <div class="flex justify-between w-full mt-auto mb-2 font-semibold text-dark text-lg/normal">
                                    <span class="mr-4 text-dark">Total Students {instructorStatistics2 && instructorStatistics2.totalStudentCount}</span>
                                    <span>{instructorStatistics && instructorStatistics2 && (instructorStatistics2.totalStudentCount/instructorStatistics.courseCount).toFixed(2)}%</span>
                                </div>

                                <div class="mx-3 rounded-2xl h-[8px] w-full bg-white">
                                <div class="rounded-2xl bg-dark w-[85%] h-[8px]" style={{ width: `${instructorStatistics && instructorStatistics2 && instructorStatistics2.totalStudentCount/instructorStatistics.courseCount}%`}}></div>
                                </div>
                            </div>
                            {/* <!--end::Progress--> */}
                        </div>
    ...
    {isChatbotVisible && (
      <div className='chatbot-container'>
        ...
        <div className="chat-window">
          <InstructorChatbot profilePhotoUrl={profilePhotoUrl}/>
        </div>
