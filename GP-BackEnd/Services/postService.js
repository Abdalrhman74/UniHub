...
const listRecentAnnouncementsService = (token, callback) => {
    jwt.verify(token, 'unihubaammy', (err, decoded) => {
        if (err) {
            callback(null, { message: 'Token verification failed', details: err, status: 401 });
        } else {
            const userId = decoded.id;

            // Proceed with the query and include the check for user registration in the course
            const query = `
                SELECT p.postName, CONCAT(u.firstName, ' ', u.lastName) AS userName, u.userType, c.courseName, p.postDate
                FROM posts p
                JOIN user u ON p.userId = u.userID
                JOIN course c ON p.courseId = c.courseID
                JOIN registeredcourses rc ON rc.courseID = c.courseID AND rc.userID = ?
            ...
            db.query(query, [userId], (err, results) => {
                ...
                if (results.length === 0) {
                    return callback(null, { message: 'No recent announcements found or user not registered in relevant courses', status: 404 });
                }
